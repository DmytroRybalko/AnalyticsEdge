...
```{r}
census_glm <- glm(over50k ~ ., data = train, family = "binomial")
summary(census_glm)
```
*Q1:Which variables are significant, or have factors that are significant?*    
(Use 0.1 as your significance threshold, so variables with a period or dot in the stars column should be counted too. You might see a warning message here - you can ignore it and proceed. This message is a warning that we might be overfitting our model to the training set.) Select all that apply.

# Problem 1.2 - A Logistic Regression Model  
  
*Q2: What is the accuracy of the model on the testing set?*  
Use a threshold of 0.5. (You might see a warning message when you make predictions on the test set - you can safely ignore it.)  
```{r}
census_glm_pred <- predict(census_glm, type = "response", newdata = test)
table(test$over50k, census_glm_pred > 0.5)
```
Accuracy:
```{r}
(9051 + 1888) / nrow(test)
...
*Q3: What is the baseline accuracy for the testing set?*  
```{r}
table(test$over50k)
```
Accuracy^
...
*Q4: What is the area-under-the-curve (AUC) for this model on the test set?*  
```{r}
ROCRpred <- prediction(census_glm_pred, test$over50k)
as.numeric(performance(ROCRpred, "auc")@y.values)
```
Buil ROC curve:
```{r}
ROClog <- performance(ROCRpred, "tpr", "fpr")
plot(ROClog)
```
...
*Q5: How many splits does the tree have in total?*
*A:* 4.  
  
# Problem 2.2 - A CART Model  
  
*Q6: Which variable does the tree split on at the first level (the very first split of the tree)?*  
*A:* Relationship.

# Problem 2.3 - A CART Model  
  
*Q7: Which variables does the tree split on at the second level (immediately after the first split of the tree)? Select all that apply.*  
*A: capitalgain, education.  
  
# Problem 2.4 - A CART Model  
  
*Q8: What is the accuracy of the model on the testing set? Use a threshold of 0.5.*   
(You can either add the argument type="class", or generate probabilities and use a threshold of 0.5 like in logistic regression.)  
```{r}
CART1_pred <- predict(CART1, newdata = test, type = "class")
table(test$over50k, CART1_pred)
```
Answer:
```{r}
(9243 + 1596) / nrow(test)
...
```{r}
CART_ROC <- predict(CART1, newdata = test)
CART_ROC[1:10,]
```
Make prediction:  
```{r}
predCARTauc <- prediction(CART_ROC[, 2], test$over50k)
```
...
```{r}
perf <- performance(predCARTauc, "tpr", "fpr")
plot(perf)
```

*Q9: Which of the following explanations for this behavior is most correct?*  
(HINT: Think about what the ROC curve is plotting and what changing the threshold does.)  
*A:*  
1) The number of variables that the logistic regression model is based on is larger than the number of variables used by the CART model, so the ROC curve for the logistic regression model will be smoother.  

2) CART models require a higher number of observations in the testing set to produce a smoother/more continuous ROC curve; there is simply not enough data.  

3) *Correct answer.* The probabilities from the CART model take only a handful of values (five, one for each end bucket/leaf of the tree); the changes in the ROC curve correspond to setting the threshold to one of those values.  

4) The CART model uses fewer continuous variables than the logistic regression model (capitalgain for CART versus age, capitalgain, capitallosses, hoursperweek), which is why the CART ROC curve is less smooth than the logistic regression one.  
...
*Q10: What is the AUC of the CART model on the test set?*  
```{r}
as.numeric(performance(predCARTauc, "auc")@y.values)
```

# Problem 3.1 - A Random Forest Model. Accuracy
...
```{r}
set.seed(1)
RF <- randomForest(over50k ~ . , data = trainSmall, method = "class")
```
Then, make predictions using this model on the entire test set.  
*Q11: What is the accuracy of the model on the test set, using a threshold of 0.5?*  
...
# Problem 3.2 - A Random Forest Model. Metric number of times, aggregated over all of the trees  
...
*Q12: Which of the following variables is the most important in terms of the number of splits?*  
*A:* age.
  
# Problem 3.3 - A Random Forest Model. "Impurity" metric  
...
varImpPlot(RF)
```
*Q13: Which one of the following variables is the most important in terms of mean reduction in impurity?*
*A:* occupation.
...
*Q14: Which value of cp does the train function recommend?*
*A:* cp = 0.002.  
...
*Q15: What is the prediction accuracy on the test set?*  
```{r}
#set.seed(3)
best.CART <- rpart(over50k ~ ., data = train, method = "class", cp = 0.002)
pred.bestCART <- predict(best.CART, newdata = test, type = "class")
table(test$over50k, pred.bestCART)
```
Accuracy:
```{r}
(9178 + 1838) / nrow(test)
...
*Q16: Plot the CART tree for this model. How many splits are there?*
```{r}
prp(best.CART)
```
