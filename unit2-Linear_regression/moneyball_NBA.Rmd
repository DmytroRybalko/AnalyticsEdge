...
So our dependent variable would now be PTS, and our independent variables would
be some of the common basketball statistics that we have in our data set.
So for example, the number of two-point field goal attempts, the number of three-point field goal attempts,
offensive rebounds, defensive rebounds, assists, steals, blocks, turnovers, free throw attempts-- we can use all of these.
...
```
It's quite a lot-- 28,394,314. So the sum of squared errors number
is not really a very interpretable quantity. But remember, we can also calculateWe can also calculate the root mean squared error, which is much more interpretable. It's more like the average error we make in our predictions.
...
summary(PointsReg3)
```
And let's take a look now at the sum of squared errors and the root mean square error, just
to make sure we didn't inflate those too much by removing a few variables.
```{r}
...
and so it's 184.5. So although we've increased the root mean squared error a little bit by removing those variables, it's really a very, very, small amount. Essentially, we've kept the root mean squared error the same.
...
Next we'll try to make predictions for the 2012-2013 season.
We'll need to load our test set because our training set only included data from 1980 up until the 2011-2012 season.
...
OK, so now that we have our prediction, how good is it? We can compute the out of sample R-squared.
...
We need to compute the sum of squared errors and so this here is just the sum of the predicted amount
...
```
So the R-squared here then is calculated as usual, 1 minus the sum of squared errors divided
by total sums of squares.
```{r}
(R2 <- 1 - SSE / SST)
```
We can also calculate the root mean squared error the same way as before, root mean squared error
is going to be the square root of the sum of squared errors divided by n, which is the number of rows in our test data
set.
```{r}
