...
*Q4: What are the values of this variable that occur in our dataset? Select all that apply.*
```{r}
...
*Q6: Out of all of the songs in our dataset, the song with the highest tempo is one of the following songs. Which one is it?*
```{r}
songs %>% 
...
```
*Q7: How many observations (songs) are in the training set?*  
```{r}
nrow(SongsTrain)
```
...
*Q9: What does the model suggest?*  
...
*Q10: What does Model 1 suggest in terms of complexity?*  
*A*: Mainstream listeners tend to prefer less complex songs  
...
*Q11: By inspecting the coefficient of the variable "loudness", what does Model 1 suggest?*  
*A*: Mainstream listeners prefer songs with heavy instrumentation.  
  
*Q12: By inspecting the coefficient of the variable "energy", do we draw the same conclusions as above?*  
*A*: no (energy coeff < 0). 
  
## Problem 3.1 - Beware of Multicollinearity Issues!  
  
*Q13: What is the correlation between the variables "loudness" and "energy" in the training set?*
```{r}
...
*Q14: Look at the summary of SongsLog2, and inspect the coefficient of the variable "energy". What do you observe?*  
```{r}
summary(SongsLog2)
```
...
*Q15: Remembering that higher loudness and energy both occur in songs with heavier instrumentation, do we make the same observation about the popularity of heavy instrumentation as we did with Model 2?*
*A*: Yes.  
...
```{r}
predictModel3 <- predict(SongsLog3, type = "response", newdata = SongsTest)
```

*Q16: What is the accuracy of Model 3 on the test set, using a threshold of 0.45? (Compute the accuracy as a number between 0 and 1.)*
```{r}
table(SongsTest$Top10, predictModel3 > 0.45)
```
Accuracy is:
```{r}
(309 + 19) / (309 + 5 + 40 + 19)
...
*Q17: What would the accuracy of the baseline model be on the test set? (Give your answer as a number between 0 and 1.)*
```{r}
...
*Q18: How many songs does Model 3 correctly predict as Top 10 hits in 2010 (remember that all songs in 2010 went into our test set), using a threshold of 0.45?*
```{r}
table(SongsTest$Top10, predictModel3 > 0.45)
```
*A*: 19.  
  
*Q19: How many non-hit songs does Model 3 predict will be Top 10 hits (again, looking at the test set), using a threshold of 0.45?*  
*A*: 5  
  
*Explanation.* According to our model's confusion matrix:  
...
*Q20: What is the sensitivity of Model 3 on the test set, using a threshold of 0.45?*  
```{r}
table(SongsTest$Top10, predictModel3 > 0.45)
```
```{r}
...
*Q21: What is the specificity of Model 3 on the test set, using a threshold of 0.45?*
```{r}
309 / (309 + 5)
```
...
*Q22: What conclusions can you make about our model? (Select all that apply.)*  
*A1:* Model 3 favors specificity over sensitivity.  
*A2:* Model 3 provides conservative predictions, and predicts that a song will make it to the Top 10 very rarely. So while it detects less than half of the Top 10 songs, we can be very confident in the songs that it does predict to be Top 10 hits.  
