...
*Q1: How many Facebook users are there in our dataset?*
```{r}
nrow(users)
```

*Q2: In our dataset, what is the average number of friends per user?*  
...
*Q3: Out of all the students who listed a school, what was the most common locale?*
```{r}
table(users$locale)
```
...
*Q4: Is it possible that either school A or B is an all-girls or all-boys school?*
```{r}
...
*Q5: Based on ?graph.data.frame, which of the following commands will create a graph g describing our social network, with the attributes of each user correctly loaded?*  

*A:* g = graph.data.frame(edges, FALSE, users). 
...
*Q6: How many connected components with at least 2 nodes are there in the graph?*
*A:* 4.

*Q7: How many users are there with no friends in the network?*
*A:* 7.
...
*Q8: How many users are friends with 10 or more other Facebook users in this network?*
*A:* 9.
...
*Q9: What is the largest size we assigned to any node in our graph?*
```{r}
max(V(g)$size)
```

*Q10: What is the smallest size we assigned to any node in our graph?*
```{r}
min(V(g)$size)
```

*Explanation.* From table(degree(g)) or summary(degree(g)), we see that the maximum degree of any node in the graph is 18 and the minimum degree of any node is 0. Therefore, the maximum size of any point is 18/2+2=11, and the minimum size is 0/2+2=2.  
...
*Q11: What is the gender of the users with the highest degree in the graph?*
*A:* Gender B.  
...
*Q12: Are the two users who attended both schools A and B Facebook friends with each other?*
*A:* Yes.

*Q13: What best describes the users with highest degree?*
*A:* Some, but not all, of the high-degree users attended school A.  
  
*Explanation.* As with coloring by gender, we will set the color for all vertices to black, and then we will set the color for students from school A to red and the color for students from schools A and B to gray.  
...
```{r}
V(g)$color <- "black"
V(g)$color[V(g)$locale == "A"] <- "yellow"
V(g)$color[V(g)$locale == "B"] <- "brown"
plot(g, vertex.label = NA)
```

*Q14: The large connected component is most associated with which locale?*  
*A:* locale B.

*Q15: The 4-user connected component is most associated with which locale?*  
*A:* locale A.  
...
*Q16: Which igraph plotting function would enable us to plot our graph in 3-D?*  
*A:* rglplot.  
  
*Q17: What parameter to the plot() function would we use to change the edge width when plotting g?*  
*A:* edge.width.  
